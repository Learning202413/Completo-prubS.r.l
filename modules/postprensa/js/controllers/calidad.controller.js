...
export const CalidadController = {
    ...
    init: async function(params) {
        ...
        if (this.currentTaskId) {
            this.taskData = LocalDB.getById(this.currentTaskId);
    ...
    async saveStep(stepKey) {
        ...
        const updates = {
            avance_postprensa: { ...this.taskData.avance_postprensa, [stepKey]: true },
        ...
        await LocalDB.update(this.currentTaskId, updates);
        this.taskData = LocalDB.getById(this.currentTaskId);
        this.checkQCVisibility();
    },
    ...
    setupEvents() {
        ...
        document.getElementById('btn-complete-order')?.addEventListener('click', async () => {
             // 1. Definir fecha y estado final
             const now = getTimestamp();
             
             // 2. Actualizar la Base de Datos Local
             await LocalDB.update(this.currentTaskId, { 
                 estado: 'Completado',          // Estado final para el CRM
                 fecha_fin_proceso: now,        // Registro de cierre
                 ultima_actualizacion: now
             });
